...
export default function ChatInput({
  ...
  const defaultAgentId = useMemo(() => {
    if (taskStage && stages.length > 0) {
      const stage = stages.find((s) => s.id === taskStage);
      if (stage?.agent_id) return stage.agent_id;
    }
    if (project?.default_agent_id) return project.default_agent_id;
    if (project?.planning_agent_id) return project.planning_agent_id;
    if (agents.length > 0) return agents[0].id;
    return null;
  }, [taskStage, stages, project, agents]);
