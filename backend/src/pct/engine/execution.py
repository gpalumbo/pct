...
def _resolve_provider(project_root: Path, agent_id: str | None) -> AgentProvider | None:
    ...
    return resolve_provider(agent_id)
...
def _build_messages(
    ...
    """Assemble the message list for an agent execution turn.

    1. Resolve task context variables.
    2. Expand the stage prompt template.
    3. Return a system message + user message with the expanded prompt.
    """
    variables = resolve_task_context(project_root, feature_id, task_id)
    ...
    messages: list[dict[str, str]] = [
        {"role": "system", "content": "You are an agent executing a project task."},
...
async def execute_task(
    ...
    Steps:
      ...
      5. On success: advance task to next stage.
      6. On failure: set execution_status=error, populate error_details.
    ...
    if provider is None:
        # No provider available — return a stub success so the scheduler can
        ...
        result = await execute_chat_turn(provider, messages)
